function [testRMSE,testMAPE]=ANN_FF_RealData2(mod,neuronas,trfcn,cont)
...
% datosTest=xlsread('Datasets/Datos_BTOmod1tiempo-SIMULACION.xlsx',1,'A390:F1264');%Exp: Test 24-42
datosTest=xlsread('Datasets/Datos_BTOmod1tiempo-SIM4.xlsx',1,'A390:F669');%Exp: Test 1-16
...
indicest=find(datosTest(:,5)>=0.9999);
[mt,nt]=size(indicest);
for i=1:mt
    if i~=mt
...
%     fprintf('Entrenamiento del modelo %d finalizado\n',i);
       ...
       modname=strcat('Results/BTOnet_cont',int2str(cont),'.mat');
       save(modname,'normVal','net');
